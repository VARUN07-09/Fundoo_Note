class User < ApplicationRecord
  ...
  # Virtual attributes for OTP and its expiry
  attr_accessor :otp, :otp_expiry

  # Generates an OTP, stores it in an in-memory store, and returns a hash with the OTP and its expiry.
  def generate_otp
    otp = rand(100000..999999).to_s
    expiry = 10.minutes.from_now
    self.class.store_otp(email, otp, expiry)  # Stores OTP for this email
    expiry_in_kolkata = expiry.in_time_zone('Asia/Kolkata')  # Convert expiry time to IST
    { otp: otp, otp_expiry: expiry_in_kolkata.to_s }
  end
  
  # Validates the entered OTP against the stored OTP and checks expiry.
  ...
  # Class-level OTP store methods (in-memory store).
  def self.otp_store
    @otp_store ||= {}  
  end
